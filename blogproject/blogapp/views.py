...
def mail_send_view(request,id):
	...
	if request.method=='POST':
		...
		if form.is_valid():
			...
			subject='{}({}) recommended to read "{}" '.format(cd['name'],cd['email'],post.title)
			post_url=request.build_absolute_uri(post.get_absolute_url())
			message='Read the post at :\n {} \n\n {}\'s Comments:\n{}'.format(post_url,cd['name'],cd['comments'])

		#send_mail(subject,message,from_mail,recipient_list)
			send_mail(subject,message,'pujarimanjunath987@gmail.com',[cd['to']])
			sent=True
	...
	return render(request=request,template_name='blogapp/sharebyemail.html',context=my_dict)
